...
const TimelineIcon: React.FC<{ type: TimelineEvent['timelineType'] }> = ({ type }) => {
    const icons = {
        ...
        communication: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" viewBox="0 0 20 20" fill="currentColor"><path d="M10.894 2.553a1 1 0 00-1.788 0l-7 14a1 1 0 001.169 1.409l5-1.428A1 1 0 009 15.571V11a1 1 0 112 0v4.571a1 1 0 00.725.962l5 1.428a1 1 0 001.17-1.408l-7-14z" /></svg>,
    };
    const colors = {
        activity: 'bg-blue-500', note: 'bg-yellow-500', bonus: 'bg-green-500', audit: 'bg-purple-500', communication: 'bg-teal-500'
    };
    return (
...
const TimelineItem: React.FC<{ event: TimelineEvent }> = ({ event }) => {
    ...
    const renderContent = () => {
        switch (event.timelineType) {
            ...
            case 'communication':
                return <><span className="capitalize">{event.type}</span> sent by <span className="font-semibold">{event.adminUsername}</span>: <span className="italic text-dark-text-secondary">"{event.templateName}"</span></>;
            default:
